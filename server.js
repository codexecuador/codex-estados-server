...
app.post("/create-user", (req, res) => {
  ...
  const checkUserSql =
    "SELECT ID FROM cdx_users WHERE user_login = ? OR user_email = ?";
  db.query(checkUserSql, [user_login, user_email], (err, results) => {
    ...
    db.query(
      insertUserSql,
      [user_login, hashedPassword, user_nicename, user_email, user_nicename],
      (err, result) => {
        if (err) {
        ...
        const insertEnterpriseSql =
          "INSERT INTO cdx_txt_enterprises (user_id, enterprises) VALUES (?, 0)";
        ...
        const insertUsermetaSql =
          "INSERT INTO cdx_usermeta (user_id, meta_key, meta_value) VALUES ?";
        db.query(insertUsermetaSql, [usermetaValues], (err) => {
          ...
          db.query(
            ...
            [newUserId, expirationDate.toISOString().slice(0, 10)],
            (err) => {
              if (err) {
...
app.post("/login", (req, res) => {
  ...
  db.query(
    ...
    async (error, results) => {
      if (error) {
        // Caso en el que no se pueda conectar con la base de datos
        res.status(500).send("Error en el servidor");
      } else if (results.length === 0) {
        // Caso en el que no se encuentra un usuario con ese nombre
        res.status(401).send("Usuario no encontrado");
      } else {
        // Caso en el que sí se encuentra un usuario con ese nombre
        ...
        if (isPasswordCorrect) {
          ...
          db.query(
            "SELECT meta_value FROM cdx_usermeta WHERE user_id = ? AND meta_key = ?",
            [user.ID, "cdx_user_level"],
            async (error, metaResults) => {
              if (error) {
                res.status(500).send("Error en el servidor");
              } else {
                const userLevel =
                  metaResults.length > 0 ? metaResults[0].meta_value : null;
...
app.post("/api/create", verifyToken, (req, res) => {
  ...
  const checkEnterpriseSql =
    "SELECT enterprises FROM cdx_txt_enterprises WHERE user_id = ?";
  db.query(checkEnterpriseSql, [userId], (err, results) => {
    ...
    const checkSubscriptionSql =
      "SELECT * FROM cdx_pms_member_subscriptions WHERE user_id = ?";
  ...
  function verificarYCrearDatos(enterpriseLimit) {
    ...
    db.query(checkSql, [userId], (err, results) => {
      ...
      if (count >= enterpriseLimit) {
        return res
          .status(403)
          .send("No se pueden crear más datos porque se alcanzó el límite.");
      }
      ...
      const insertSql =
        "INSERT INTO cdx_txt (user_id, yearCurrent, yearPrevious, personalData, downloads) VALUES (?, ?, ?, ?, ?)";
      db.query(
        insertSql,
        [
          userId,
...
app.get("/api/data", verifyToken, (req, res) => {
  ...
  db.query(sql, [userId, dataId], (err, result) => {
    ...
    if (expirationDate < currentDate) {
      return res
        .status(403)
        .json({ valid: false, message: "Su suscripción ha expirado." });
    }
...
app.get("/api/all-users", verifyToken, (req, res) => {
  ...
  const userInfoQuery = `
    ...
    WHERE ${
      isEmailSearch ? "cdx_users.user_email" : "cdx_users.user_login"
    } LIKE ?
    LIMIT ? OFFSET ?
  `;
  ...
  db.query(
    ...
    (err, userInfoResult) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Error interno al obtener información del usuario" });
      }
    ...
    `;
      ...
      db.query(totalQuery, [searchPattern], (err, totalResult) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Error interno al contar usuarios" });
        }
...
`;
  ...
  db.query(userQuery, [userId], (err, userResult) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error interno al obtener usuario" });
    }
    ...
    db.query(personalDataQuery, [userId], (err, personalDataResult) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Error interno al obtener datos personales" });
      }
      ...
      db.query(paymentsQuery, [userId], (err, paymentsResult) => {
        ...
        db.query(enterpriseQuery, [userId], (err, enterpriseResult) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error interno al obtener empresa" });
          }

          const enterprise =
            (enterpriseResult && enterpriseResult[0]?.enterprises) || 0;
          ...
          db.query(subscriptionQuery, [userId], (err, subscriptionResult) => {
            if (err) {
              ...
              return res
                .status(500)
                .json({ error: "Error interno al obtener suscripción" });
            }

            const subscription = subscriptionResult.map(
...
app.get("/api/all-data", verifyToken, (req, res) => {
  ...
  const sqlUserData =
    "SELECT ID, yearCurrent, personalData FROM cdx_txt WHERE user_id = ?";
  const sqlEnterprise =
    "SELECT enterprises FROM cdx_txt_enterprises WHERE user_id = ?";
  const sqlInsertEnterprise =
    "INSERT INTO cdx_txt_enterprises (user_id, enterprises) VALUES (?, ?)";
  const sqlSuscription =
    "SELECT * FROM cdx_pms_member_subscriptions WHERE user_id = ?";
      ...
      if (enterpriseData.length === 0) {
        ...
        db.query(sqlInsertEnterprise, [userId, 0], (insertErr) => {
          ...
          db.query(sqlEnterprise, [userId], (fetchErr, newEnterpriseData) => {
            ...
            const enterprises =
              newEnterpriseData[0].enterprises + extraEnterprises;
            sendResponse(
              userId,
              userName,
              userLevel,
...
app.delete("/api/delete/:userId/:enterpriseId", verifyToken, (req, res) => {
  ...
  db.query(deleteQuery, [userId, enterpriseId], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error interno al eliminar la fila" });
    }

    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ error: "Fila no encontrada o ya eliminada" });
    }
...
app.put("/api/update-enterprises", verifyToken, (req, res) => {
  ...
  db.query(checkQuery, [userId], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error interno al verificar el usuario" });
    }
...
app.put("/api/update-company", verifyToken, (req, res) => {
  ...
  const updateSql =
    "UPDATE cdx_txt SET yearCurrent = ?, yearPrevious = ?, personalData = ? WHERE user_id = ?  AND ID = ?";
  const personalDataSerialized = JSON.stringify(personalData);
  db.query(
    updateSql,
    [yearCurrent, yearPrevious, personalDataSerialized, userId, dataId],
    (err, result) => {
      if (err) {
...
app.get("/api/get-requests", verifyToken, (req, res) => {
  const sql =
    "SELECT ID, personalData, requestChange FROM cdx_txt WHERE requestChange IS NOT NULL AND requestApproved IS NOT TRUE";
...
app.post("/api/send-request", verifyToken, (req, res) => {
  ...
  const updateSql =
    "UPDATE cdx_txt SET requestChange = ?, requestApproved = ? WHERE user_id = ?  AND ID = ?";
...
app.put("/api/check-request", verifyToken, (req, res) => {
  ...
  const updateSql =
    "UPDATE cdx_txt SET personalData = ?, requestChange = ?, requestApproved = ? WHERE ID = ?";

  const personalDataSerialized = JSON.stringify(personalData);

  db.query(
    updateSql,
    [personalDataSerialized, null, approvalStatus, dataId],
    (err, result) => {
      if (err) {
        console.error("Error al actualizar datos: " + err.message);
        return res.status(500).send("Error interno del servidor");
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Error al verificar clientTransactionId" });
      }
